#!/usr/bin/env python3
"""
SC Standard CBAM CNN训练脚本
专门用于训练CBAM注意力增强的回环检测模型
"""
...
from models.sc_standard_cbam_cnn import SCStandardCBAMCNN
...
class SCStandardCBAMTrainer:
    """SC Standard CBAM CNN训练器"""
    ...
    def __init__(self, config):
        ...
        self.logger = setup_logger(
            f"sc_standard_cbam_trainer", 
            f"training_sc_standard_cbam_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
        
        self.logger.info(f"🚀 初始化SC Standard CBAM训练器")
    ...
    def _save_best_model(self, epoch, metric_type):
        """保存最佳模型"""
        os.makedirs("outputs/sc_standard_cbam/models", exist_ok=True)
        
        model_path = f"outputs/sc_standard_cbam/models/best_{metric_type}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pth"
    ...
    def train(self):
        """开始训练"""
        self.logger.info(f"🎯 开始SC Standard CBAM训练 - {self.config['epochs']}轮")
        ...
        self.logger.info(f"🎉 SC Standard CBAM训练完成! 总用时: {total_hours:.2f}小时")
    ...
    def _save_results(self, total_time):
        """保存训练结果"""
        os.makedirs("outputs/sc_standard_cbam/results", exist_ok=True)
        
        results = {
            'model_name': 'sc_standard_cbam',
            'config': self.config,
            'best_map': self.best_map,
            'best_top1': self.best_top1,
        ...
        results_path = f"outputs/sc_standard_cbam/results/training_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    parser = argparse.ArgumentParser(description='SC Standard CBAM CNN训练')
    ...
    parser.add_argument('--reduction', type=int, default=16, help='CBAM缩减比例')
    ...
    print(f"🚀 开始训练SC Standard CBAM模型")
    print(f"📊 配置: {config}")
    
    trainer = SCStandardCBAMTrainer(config)
