...
def train_epoch(model, dataloader, criterion, optimizer, device, logger):
    ...
    progress_bar = tqdm(dataloader, desc="Training SCRingCNN")
    ...
    logger.info(f"SCRingCNN Training Loss: {avg_loss:.4f}")
    return avg_loss
...
def evaluate(model, dataloader, device, logger):
    ...
    with torch.no_grad():
        for data, labels in tqdm(dataloader, desc="Evaluating SCRingCNN"):
            data = data.to(device)
            features = model(data)
    ...
    # 记录结果
    logger.info("SCRingCNN Evaluation Results:")
...
def main():
    parser = argparse.ArgumentParser(description='SCRingCNN专门训练脚本')
    ...
    parser.add_argument('--data_dir', type=str, default=None,
                       help='数据目录路径')
    parser.add_argument('--max_files', type=int, default=600,
                       help='最大文件数量')
    ...
    logger, log_file = setup_model_logger(
        model_type='sc_ring_cnn',
        script_type='training',
        timestamp=timestamp,
        project_root=project_root
    )
    
    logger.info("🚀 开始SCRingCNN专门训练")
    ...
    logger.info(f"SCRingCNN模型参数数量: {sum(p.numel() for p in model.parameters()):,}")
    ...
    best_top1 = 0
    results = {
        'model_type': 'SCRingCNN',
        'experiment_name': f'sc_ring_cnn_{timestamp}',
        'config': vars(args),
        'epochs': [],
        'best_metrics': None
    }
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 2 == 0 or epoch == args.epochs - 1:
            ...
            if metrics['top_1'] > best_top1:
                ...
                model_path = model_dir / f"best_sc_ring_cnn_{timestamp}.pth"
                torch.save({
                ...
                logger.info(f"保存最佳SCRingCNN模型: {model_path}")
    
    # 保存训练结果
    results_dir = project_root / "outputs" / "results"
    results_dir.mkdir(parents=True, exist_ok=True)
    
    results_path = results_dir / f"sc_ring_cnn_results_{timestamp}.json"
    with open(results_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=2, ensure_ascii=False, default=str)
    
    logger.info(f"SCRingCNN训练完成！结果保存至: {results_path}")
