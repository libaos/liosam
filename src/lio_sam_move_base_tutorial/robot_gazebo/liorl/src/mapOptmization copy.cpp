#include "utility.h"
#include "liorl/cloud_info.h"
#include "liorl/save_map.h"
// <!-- liorl_yjz_lucky_boy -->
...
public:
    ...
    mapOptimization()
    {
        ...
        pubKeyPoses                 = nh.advertise<sensor_msgs::PointCloud2>("liorl/mapping/trajectory", 1);
        pubLaserCloudSurround       = nh.advertise<sensor_msgs::PointCloud2>("liorl/mapping/map_global", 1);
        pubLaserOdometryGlobal      = nh.advertise<nav_msgs::Odometry> ("liorl/mapping/odometry", 1);
        ...
        subCloud = nh.subscribe<liorl::cloud_info>("liorl/deskew/cloud_info", 1, &mapOptimization::laserCloudInfoHandler, this, ros::TransportHints().tcpNoDelay());
        ...
        srvSaveMap  = nh.advertiseService("liorl/save_map", &mapOptimization::saveMapService, this);

        pubHistoryKeyFrames   = nh.advertise<sensor_msgs::PointCloud2>("liorl/mapping/icp_loop_closure_history_cloud", 1);
        pubIcpKeyFrames       = nh.advertise<sensor_msgs::PointCloud2>("liorl/mapping/icp_loop_closure_corrected_cloud", 1);
        pubLoopConstraintEdge = nh.advertise<visualization_msgs::MarkerArray>("/liorl/mapping/loop_closure_constraints", 1);

        pubRecentKeyFrames    = nh.advertise<sensor_msgs::PointCloud2>("liorl/mapping/map_local", 1);
        pubRecentKeyFrame     = nh.advertise<sensor_msgs::PointCloud2>("liorl/mapping/cloud_registered", 1);
        pubCloudRegisteredRaw = nh.advertise<sensor_msgs::PointCloud2>("liorl/mapping/cloud_registered_raw", 1);
        pubGlobalMap          = nh.advertise<sensor_msgs::PointCloud2>("liorl/localization/global_map", 1);

        pubSLAMInfo           = nh.advertise<liorl::cloud_info>("liorl/mapping/slam_info", 1);
        pubGpsOdom            = nh.advertise<nav_msgs::Odometry> ("liorl/mapping/gps_odom", 1);
        // 发布GPS话题
        pubOptGPS = nh.advertise<sensor_msgs::NavSatFix>("liorl/mapping/gps_optimized", 10);
    ...
    void laserCloudInfoHandler(const liorl::cloud_infoConstPtr& msgIn)
    {
        // extract time stamp
     ...
     bool systemInitialize()
    {
        ...
        icp.setMaxCorrespondenceDistance(32);
        icp.setMaximumIterations(512);
        ...
        *cloudOut += *transformPointCloud(laserCloudSurfLast, &thisPose6D);
        ...
        {
            ROS_ERROR("initialize pose failed. Fitness score : %f", icp.getFitnessScore());
    ...
    void gpsHandler(const sensor_msgs::NavSatFixConstPtr& gpsMsg)
    {
        ...
        if (!first_gps) {
            ...
            /*设置局部笛卡尔坐标系原点*/
            ros::param::set("origin_x", gpsMsg->latitude);
            ros::param::set("origin_y", gpsMsg->longitude);
            ros::param::set("origin_z", gpsMsg->altitude);
            ROS_INFO("position: %f, %f, %f", gpsMsg->latitude, gpsMsg->longitude, gpsMsg->altitude);
        }
    ...
    bool saveMapService(liorl::save_mapRequest& req, liorl::save_mapResponse& res)
    {
    ...
    void visualizeGlobalMapThread()
    {
        ...
        liorl::save_mapRequest  req;
        liorl::save_mapResponse res;
...
/*             float arx = (crx*sry*srz*pointOri.x + crx*crz*sry*pointOri.y - srx*sry*pointOri.z) * coeff.x
    ...
    void addGPSFactor()
    {
        ...
        {
            if (gpsQueue.front().header.stamp.toSec() < timeLaserInfoCur - 0.30)
            {
                // message too old
                // ROS_INFO("message too new");
                gpsQueue.pop_front();
            }
            else if (gpsQueue.front().header.stamp.toSec() > timeLaserInfoCur + 0.30)
            {
                // message too new
                // ROS_INFO("message too new");
                break;
            ...
            else
            {
                ...
                ROS_INFO(" Add GPS factor successfully!!!");
                aLoopIsClosed = true;
                break;
            }
        }
    ...
    void publishOdometry()
    {
        ...
        /*publish optimized_gps topic*/
        Eigen::Vector3d odomTrans;
        sensor_msgs::NavSatFix gpsMsg;
        odomTrans << laserOdometryROS.pose.pose.position.x, laserOdometryROS.pose.pose.position.y, laserOdometryROS.pose.pose.position.z;

        // 转换到经纬度
        gps_trans_.Reverse(odomTrans[0], odomTrans[1], odomTrans[2], gpsMsg.latitude, gpsMsg.longitude, gpsMsg.altitude);
        gpsMsg.header.stamp = timeLaserInfoStamp;
...
{
    ros::init(argc, argv, "liorl");

    mapOptimization MO;

    /*初始化局部笛卡尔坐标系原点*/
    ros::param::set("origin_x", 0);
    ros::param::set("origin_y", 0);
    ros::param::set("origin_z", 0);
