...
# Support both:
#   run_orchard_dwa_server.sh PORT [PATH_JSON] [WORLD_FILE] [MODE]
# and a short form:
#   run_orchard_dwa_server.sh PORT MODE
case "${2:-}" in
...
if [[ -z "${RECORD_CSV:-}" ]]; then
  case "${MODE}" in
    velodyne)
      RECORD_CSV="${WS_DIR}/trajectory_data/dwa_odom.csv"
      ;;
    no_obstacles)
      RECORD_CSV="${WS_DIR}/trajectory_data/dwa_no_obstacles_odom.csv"
      ;;
    static_map)
      RECORD_CSV="${WS_DIR}/trajectory_data/dwa_static_odom.csv"
      ;;
  esac
fi
...
exec roslaunch pcd_gazebo_world orchard_dwa_replay.launch \
  gui:=false \
